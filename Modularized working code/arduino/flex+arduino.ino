...
const int FLEX_SAMPLES = 5;              // Moving average window size (100ms)
const int SPIKE_THRESHOLD = 15;          // Percent - reject jumps larger than this

// Flex sensor state
int flexReadings[FLEX_SAMPLES] = {0};    // Moving average buffer
int flexIndex = 0;                       // Current position in buffer
int flexTotal = 0;                       // Running total for average
int lastValidFlex = 0;                   // Last valid flex percentage (for spike rejection)

int currentFlexPercent = 0;              // Updated every 20ms (internal, filtered)
...
int readAndProcessFlex() {
  ...
  // 2. Constrain to calibrated range (like working code does)
  raw = constrain(raw, FLAT_ADC, BENT_ADC);
  
  // 4. Apply moving average filter (reduce noise)
  flexTotal -= flexReadings[flexIndex];
  flexReadings[flexIndex] = raw;
  flexTotal += raw;
  flexIndex = (flexIndex + 1) % FLEX_SAMPLES;
  int smoothed = flexTotal / FLEX_SAMPLES;
  
  // 5. Normalize to 0.0 - 1.0
  float normalized = (float)(smoothed - FLAT_ADC) / (float)(BENT_ADC - FLAT_ADC);
  
  // 6. Apply exponential curve (power of 2.5 for more natural feel)
  float curved = pow(normalized, 2.5);
  
  // 7. Convert to percentage (0-100)
  int percentage = (int)(curved * 100.0);
  percentage = constrain(percentage, 0, 100);
  
  // 8. Spike rejection - ignore sudden jumps >15%
  if (lastValidFlex != 0) {  // Skip check on first reading
    int delta = abs(percentage - lastValidFlex);
    if (delta > SPIKE_THRESHOLD) {
      // Spike detected - reject this reading, return last valid value
      return lastValidFlex;
    }
  }
  
  // Valid reading - update last valid tracker
  lastValidFlex = percentage;
  return percentage;
}
...
void setup() {
  ...
  // Initialize flex sensor moving average buffer
  for (int i = 0; i < FLEX_SAMPLES; i++) {
    flexReadings[i] = FLAT_ADC;  // Initialize with flat position
  }
  flexTotal = FLAT_ADC * FLEX_SAMPLES;
...
void loop() {
  ...
  if (millis() - previousMillis >= SENSOR_READ_INTERVAL) {
    ...
    // ========================================================================
    // ALWAYS read and process flex sensor (every 20ms for smoothing)
    // ========================================================================
